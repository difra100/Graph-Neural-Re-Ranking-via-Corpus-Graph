...
from src.sparse_softmax import Sparsemax
...
class GNN_Glob(torch.nn.Module):
    def __init__(self, args, input_features, device):
        super(GNN_Glob, self).__init__()
        self.args = args
        ...
        self.nhid = self.args.hidden_dim
        self.num_classes = self.args.hidden_dim
        self.pooling_ratio = self.args.pooling_ratio
        self.dropout_ratio = self.args.dropout_prob
        self.sample = True
        self.sparse = True
        self.sl = self.args.structure_learning
    ...
    def forward(self, x, edge_index):
        ...
        if self.args.score:
            scores =  torch.arange(0, x.shape[0], 1, device = self.dev)
            x = torch.cat((x, scores.unsqueeze(-1)), dim=-1)
