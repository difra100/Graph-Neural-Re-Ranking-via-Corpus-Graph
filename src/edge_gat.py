...
class EdgeGATMultiplicativeConv(MessagePassing):
    def __init__(
        self,
...
class EdgeGATConv(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        # transformations 'lin_src' and 'lin_dst' to source and target nodes:
        self.lin = self.lin_src = self.lin_dst = None
        self.lin_att = self.lin_att_src = self.lin_att_dst = None
        if isinstance(in_channels, int):
            self.lin = Linear(
                in_channels,
                heads * out_channels,
        ...
        self.final_att = Linear(
            out_channels, 1, bias=False, weight_initializer="glorot"
        )
    ...
    def reset_parameters(self):
        ...
        if self.lin_att_src is not None:
            self.lin_att_src.reset_parameters()
        if self.lin_att_dst is not None:
            self.lin_att_dst.reset_parameters()
        self.final_att.reset_parameters()
        zeros(self.bias)
    ...
    def edge_update(
        ...
        # Handle both bipartite and non-bipartite cases
        if hasattr(self, 'lin_att_src') and self.lin_att_src is not None:
            # Bipartite mode
            x = self.lin_att_src(alpha_j) + self.lin_att_dst(alpha_i)
            x = x.view(-1, self.heads, self.out_channels)
        else:
            # Non-bipartite mode - use the shared attention layer
            # For non-bipartite, we need to combine alpha_j and alpha_i differently
            # One common approach is to concatenate them
            combined = torch.cat([alpha_j, alpha_i], dim=-1)
            # But since we only have one linear layer, we need to adapt
            # Alternative: use the same linear for both and sum
            x = self.lin_att(alpha_j) + self.lin_att(alpha_i)
            x = x.view(-1, self.heads, self.out_channels)

        if edge_attr is not None:
            if edge_attr.dim() == 1:
                edge_attr = edge_attr.view(-1, 1)
            assert self.lin_att_edge is not None
            edge_attr = self.lin_att_edge(edge_attr)
            edge_attr = edge_attr.view(-1, self.heads, self.out_channels)
            x = x + edge_attr

        x = F.leaky_relu(x, self.negative_slope)
        alpha = self.final_att(x).squeeze(-1)
        alpha = softmax(alpha, index, ptr, dim_size)
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)
        return alpha
