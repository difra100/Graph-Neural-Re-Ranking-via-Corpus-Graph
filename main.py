...
parser.add_argument("--sweep", type = bool, default = False, help = 'Whether to run a sweep or not, in that case choose the hyperparameter space properly.')
...
parser.add_argument("--wb", type = bool, default = False, help = 'Whether to log data into wandb.')
parser.add_argument("--save_best_model", type = bool, default = False, help = 'Whether to save the best model after training.')
...
parser.add_argument("--fast_train", type = bool, default = False, help = 'Whether to use pre-compued data to speed up the datamodule. This is bounded to about 2000 qids.')
...
parser.add_argument("--score", type = bool, default = score, help = "Whether to use the GNN with the initial bm25 as features.")
...
parser.add_argument("--heads", type = bool, default = 1, help = 'Number of heads to use in the GAT convolution.')
...
parser.add_argument("--structure_learning", type = bool, default = False, help = 'If enabling or not the structure learning mode of HGPSL.')
...
if dataset_name == 'msmarco_data':
    ...
    train_indices = list(train_set.get_topics().qid)[:length_train]
...
if not sweep:
    ...
    for seed_n in range(n_seeds): #len(seed_list)//2):
        ...
        if dataset_name == 'msmarco_data':
            ...
            pl_dataset = DataModule_terrier(train_path, val_path, batch_size, K_cg = K_cg, fast_train = fast_train, fast = True, mode = mode, embedding_name = embedding_name, train_indices = train_indices, val_indices = val_indices)
        ...
        trainer = pl.Trainer(
            ...
            default_root_dir=tot_dir, callbacks=[compute_metrics, early_stop, checkpoint_callback], deterministic = True if device == 'cpu' else False, logger = wandb_logger 
        )

        pl_training_module = TrainingModule(model, lr, wd, aggr, model_family = conv_type, dataset_name=dataset_name, qrels_folder = qrels_path, fast_train = fast_train, K_cg = K_cg, eval_indices = val_indices, loss_type = loss_type, exp_name = exp_name)
        ...
        if diz_test[nDCG@10] > best_metric and save_best_model == True:
            ...
            print("Model saved!")
        ndcg_list.append(diz_test[nDCG@10])
        if wb: 
            wandb.log({f"nDCG@{str(10)} on test ({seed_list[seed_n]})": diz_test[nDCG@10]})
...
def compute_runs(config):
    ...
    for seed_n in range(n_seeds): #len(seed_list)//2):
        ...
        if dataset_name == 'msmarco_data':
            ...
            pl_dataset = DataModule_terrier(train_path, val_path, batch_size, K_cg = config.K_cg, fast_train = fast_train, fast = True, mode = mode, embedding_name = embedding_name, train_indices = train_indices, val_indices = val_indices)
        ...
        pl_training_module = TrainingModule(model, lr = config.lr, wd = config.wd, aggr = config.aggr, model_family = config.conv_type, dataset_name=dataset_name, fast_train = fast_train, qrels_folder = qrels_path, K_cg = config.K_cg, eval_indices = val_indices, loss_type = config.loss_type, exp_name = exp_name)
        trainer.fit(model=pl_training_module, datamodule=pl_dataset)
